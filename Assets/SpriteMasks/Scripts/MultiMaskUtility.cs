...
namespace SpriteMask
{
    public static class MultiMaskUtility
    {
        ...
        {
            var dataStep = multiMask.Type.IsHard() ? 7 : 6;
        ...
        public static void RemoveMask(this List<IMultiMask> masks, float[] data, List<int> rectCounter, IMultiMask multiMask)
        {
            ...
            {
                var dataLength = data.Length;
                var ri = dataLength - (rectId << 2);
                var rightEnd = dataLength - (rectCount << 2);

                Array.ConstrainedCopy(data, ri, data, rightEnd, 4);
                rectCounter[counterId] = rectCounter[rectCount];

                for (int i = 7, c = rectCounter[counterId]; i <= leftEnd; i += 6)
                {
            ...
            rectCounter.RemoveAt(rectCount - 1);
        }
